...
def normalize_job(raw: dict[str, Any], generated_at: datetime) -> dict[str, Any]:
    ...
    first_seen_at = generated_at.astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
    ...
    return {
        ...
        "publishedAt": (
            published_at.astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
            if published_at
            else None
        ),
        "firstSeenAt": first_seen_at,
        "url": _clean(raw.get("url")),
        "directUrl": bool(raw.get("direct_url", False)),
        "sourceWarnings": [
